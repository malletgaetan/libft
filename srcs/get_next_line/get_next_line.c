...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/02 01:04:25 by gmallet           #+#    #+#             */
/*   Updated: 2023/05/07 18:02:20 by gmallet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_file	*files[MAX_OPENED_FD];
	t_string		line;
	int				error;
	...
	line.len = 0;
	if (!(files[fd]))
		files[fd] = init_file_struct();
	error = fill_line(fd, files[fd], &line);
	if (!(line.str) || error)
	{
		free(line.str);
		free(files[fd]->buf);
